...
/** Custom ski resort marker using an SVG pin. */
function skiIcon() {
  return {
...
export default function SkiMap({ resort, onGoogleLoaded }) {
  ...
  const center = { lat: resort.latitude, lng: resort.longitude };
  ...
  // Pan to new resort when selection changes
  useEffect(() => {
  ...
  // Radar overlay — re-rendered whenever current frame or radar toggle changes
  useEffect(() => {
  ...
  // Sync map type button to actual map instance
  useEffect(() => {
  ...
  return (
    <div>
      ...
      {/* Radar timeline — shown when radar is on and frames are loaded */}
